    ...
    def train(self, training_inputs, training_outputs, iterations):
        ...
        for iters in range(iterations):
            ...
            for inp, outp in zip(training_inputs, training_outputs):
                ...
                diff = np.array(training_output * np.log())
...
def main():
    tr_i, tr_o = sample_points(1000)
    tr_o = np.array([[o] for o in tr_o])
    nn = NeuralNetwork(2, [50], 1) # initialize network
    nn.train(tr_i, tr_o, 100) # train the network
    
    test_i, test_o = sample_points(1000)
    correct = 0
    for i in range(len(test_i)):
        out = nn.feed_forward(test_i[i])
        out = 1 if out[0] >= 0.5 else 0
        print('network thinks this is of class : ', 1 )
        print('and it is really of class: ', test_o[i], '\n')
        if out == test_o[i]: correct += 1
    print('network was correct on ', correct, '/', len(test_i), ' test cases')
